...
check_dependencies() {
    local missing_deps=""
    local optional_deps="ip ss netstat route"
    ...
    if [[ -n "$missing_deps" ]]; then
        echo "Warning: Missing optional dependencies for network stats plugin: $missing_deps" >&2
        echo "Network statistics collection may use fallback methods" >&2
    fi
}
